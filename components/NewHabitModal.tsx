...
function parseStartDate(input: string) {
  const trimmed = input.trim();
  if (!trimmed)
    return { ok: false as const, date: null, error: "Start date is required." };

  // Accept: ISO, "YYYY-MM-DD", or "YYYY-MM-DD HH:mm"
  const asIso = new Date(trimmed);
  if (!isNaN(asIso.getTime()))
    return { ok: true as const, date: asIso, error: null };
  ...
  if (isNaN(date.getTime())) {
    return { ok: false as const, date: null, error: "Invalid date." };
  }

  return { ok: true as const, date, error: null };
}
